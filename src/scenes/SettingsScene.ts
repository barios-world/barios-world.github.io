...
export class SettingsScene extends Phaser.Scene {
  ...
  create() {
    ...
    const pw = Math.min(w * 0.82, 340 * u), ph = Math.min(h * 0.92, 250 * u), px = (w - pw) / 2, py = (h - ph) / 2;
    ...
    this.add.text(w / 2, y - 6 * u, 'Assist: 5 Herzen, langsamere Mobs, unendlich Leben', HAND(u, 10, '#A79C90')).setOrigin(0.5, 0);
